...
func RegisterUpdateHandlers(dispatcher *tg.UpdateDispatcher, deps UpdateHandlerDeps) {
	if dispatcher == nil { return }
	logger := deps.Logger
	if logger == nil { logger = zap.NewNop() }
	dispatcher.OnNewMessage(func(ctx context.Context, e tg.Entities, update *tg.UpdateNewMessage) error {
		if deps.IsShuttingDown != nil && deps.IsShuttingDown() { return nil }
		if deps.CacheEntities != nil { deps.CacheEntities(e) }
		msg, ok := update.Message.(*tg.Message)
		if !ok || msg.Out { return nil }
		var senderID int64
		if user, ok := msg.FromID.(*tg.PeerUser); ok { senderID = user.UserID } else if user, ok := msg.PeerID.(*tg.PeerUser); ok { senderID = user.UserID }
		...
		if deps.RateLimiter != nil && !deps.RateLimiter.Allow(senderID, "command") {
			logger.Warn("assistant: rate limit exceeded for command", zap.Int64("sender_id", senderID)); return nil
		}
		if deps.CmdRouter == nil { return nil }
		err = deps.CmdRouter.Dispatch(ctx, senderID, inputPeer, msg.Message, deps.Interaction)
		if err != nil && !errors.Is(err, command.ErrUnknownCommand) { logger.Warn("assistant: command error", zap.Error(err), zap.Int64("sender_id", senderID)) }
		return nil
	})
	dispatcher.OnBotInlineQuery(func(ctx context.Context, e tg.Entities, update *tg.UpdateBotInlineQuery) error {
		if deps.IsShuttingDown != nil && deps.IsShuttingDown() { return nil }
		if deps.CacheEntities != nil { deps.CacheEntities(e) }
		if deps.RateLimiter != nil && !deps.RateLimiter.Allow(update.UserID, "inline") { logger.Warn("assistant: rate limit exceeded for inline", zap.Int64("user_id", update.UserID)); return nil }
		return nil
	})
	dispatcher.OnInlineBotCallbackQuery(func(ctx context.Context, e tg.Entities, update *tg.UpdateInlineBotCallbackQuery) error {
		...
		if deps.RateLimiter != nil && !deps.RateLimiter.Allow(update.UserID, "inline") {
			if deps.Interaction != nil { _ = deps.Interaction.Answer(ctx, update.QueryID, "Too many requests. Please wait.", true) }
			return nil
		}
		...
		if parseErr != nil {
			if deps.Interaction != nil { _ = deps.Interaction.Answer(ctx, update.QueryID, "Invalid callback", false) }
			return nil
		}
	...
	dispatcher.OnBotCallbackQuery(func(ctx context.Context, e tg.Entities, update *tg.UpdateBotCallbackQuery) error {
		...
		if deps.Resolver != nil {
			...
			if err != nil {
				if deps.Interaction != nil { _ = deps.Interaction.Answer(ctx, update.QueryID, "Unable to resolve chat. Please retry.", false) }
				return nil
			}
		}
...
func dispatchCallbackSafely(ctx context.Context, router *callback.Router, tx *callback.Transaction, logger *zap.Logger) (err error) {
	if router == nil || tx == nil { return callback.ErrUnknownAction }
	defer func() {
		if recovered := recover(); recovered != nil {
			tx.SetState(callback.StateFailed)
			err = fmt.Errorf("callback handler panic: %v", recovered)
			if logger != nil { logger.Error("assistant: callback panic recovered", zap.Int64("query_id", tx.QueryID), zap.String("namespace", tx.Payload.Namespace), zap.String("action", tx.Payload.Action), zap.Any("panic", recovered)) }
		}
	}()
...
func dispatchInlineSafely(ctx context.Context, router *callback.Router, tx *callback.InlineTransaction, logger *zap.Logger) (err error) {
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			...
			if logger != nil { logger.Error("assistant: inline callback panic recovered", zap.Int64("query_id", tx.QueryID), zap.String("namespace", tx.Payload.Namespace), zap.String("action", tx.Payload.Action), zap.Any("panic", recovered)) }
		}
	}()
...
func extractChatID(p tg.PeerClass) int64 {
	if p == nil { return 0 }
	switch peer := p.(type) {
	case *tg.PeerUser: return peer.UserID
	case *tg.PeerChat: return peer.ChatID
	case *tg.PeerChannel: return peer.ChannelID
	default: return 0
	}
}
