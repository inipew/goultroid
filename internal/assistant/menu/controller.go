...
func buildHelpPage(botUsername string, cmds []core.Command, page int) *Screen {
	...
	if len(cmds) == 0 {
		...
		card.AddField("Commands", strconv.Itoa(len(cmds))).AddField("Modules", strconv.Itoa(len(categories))).WithFooter("<i>Tap a module to browse its commands. Tap a command to see its full help.</i>")
	}
	screen := NewScreen(ScreenIDHelp, "📚 Help", card.Render())
	for i := start; i < end; i++ {
		category := categories[i]
		screen.AddRow(NewButton(fmt.Sprintf("📂 %s · %d", category, len(commandsInCategory(cmds, category))), "a1:assistant:help_module:"+strconv.Itoa(i)))
	}
	if pages > 1 {
...
func buildHelpModulePage(cmds []core.Command, moduleIndex, page int) *Screen {
	...
	for i := start; i < end; i++ {
		...
		screen.AddRow(NewButton(label, "a1:assistant:help_command:"+fmt.Sprintf("%d:%d", moduleIndex, i)))
	}
	if pages > 1 {
...
func helpPagerButtons(action string, page, pages int) ui.ButtonRow {
	row := ui.ButtonRow{}
	if page > 0 {
		row = append(row, NewButton("◀️ Previous", "a1:"+action+":"+strconv.Itoa(page-1)))
	}
	row = append(row, NewButton(fmt.Sprintf("%d / %d", page+1, pages), "a1:assistant:noop"))
	if page+1 < pages {
		row = append(row, NewButton("Next ▶️", "a1:"+action+":"+strconv.Itoa(page+1)))
	}
	return row
}
...
func BuildRetiredShellScreen() *Screen {
	card := ui.NewCard("Legacy Assistant menu retired").
		WithIcon("ℹ️").
		WithHeader("This Assistant shell has moved to the current a2 interaction runtime.").
		WithRaw("Send <code>/start</code> to open the current Assistant shell. Existing plugin-owned compatibility menus remain available while their migrations complete.").
		WithFooter("<i>This legacy shell message no longer opens new a1 navigation.</i>")
	screen := NewScreen(ScreenIDStart, "", card.Render())
	screen.AddRow(NewButton("❌ Close", "a1:assistant:close"))
	return screen
}
...
func (c *Controller) AttachRoutes(r *callback.Router, getUsername func() string, getStartTime func() time.Time) {
	...
	retired := func(ctx context.Context, tx *callback.Transaction) error {
		if _, err := c.validateSession(ctx, tx); err != nil {
			return err
		}
		unlock := c.lockInstance(tx)
		defer unlock()
		return c.editScreen(ctx, tx, BuildRetiredShellScreen())
	}
	for _, action := range []string{
		...
		r.Register("assistant", action, retired)
	}
