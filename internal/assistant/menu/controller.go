...
func (c *Controller) AttachRoutes(r *callback.Router, getUsername func() string, getStartTime func() time.Time) {
	...
	r.Register("assistant", "help_module", func(ctx context.Context, tx *callback.Transaction) error {
		...
		moduleIndex, err := strconv.Atoi(tx.Payload.State)
		if err != nil {
			return fmt.Errorf("invalid help module: %w", err)
		}
		unlock := c.lockInstance(tx)
		defer unlock()
		text, markup := c.renderer(buildHelpModulePage(commands(), moduleIndex, 0))
		return tx.Edit(ctx, text, markup)
	})
	...
	r.Register("assistant", "noop", func(ctx context.Context, tx *callback.Transaction) error { return tx.Answer(ctx, "", false) })
