...
// ScreenContext is the dynamic input shared by all automatically registered screens.
...
func BuildHelpScreenWithCommands(botUsername string, cmds []core.Command) *Screen {
	...
	if len(cmds) == 0 {
		...
		screen.Body = ui.NewCard("Assistant Commands").
			WithIcon("📚").
			WithHeader(fmt.Sprintf("%d commands are available on the Assistant surface.", len(sorted))).
			WithRaw(strings.TrimSpace(b.String())).
			WithFooter("<i>Use /help &lt;command&gt; for detailed documentation.</i>").
			Render()
	}
...
func BuildStatusScreen(botUsername string, uptime time.Duration, engine string) *Screen {
	...
	if engine == "" { engine = "GoUltroid (MTProto)" }
	...
	screen.AddRow(NewButton("🔄 Refresh", "a1:assistant:status"), NewButton("🏠 Home", "a1:assistant:start"))
	screen.AddRow(NewButton("❌ Close", "a1:assistant:close"))
	return screen
}

func (c *Controller) lockInstance(tx *callback.Transaction) func() {
	if c.instances != nil && tx != nil { return c.instances.LockInstance(tx.Target.ChatID(), tx.Target.MessageID()) }
	return func() {}
}
...
func (c *Controller) validateSession(ctx context.Context, tx *callback.Transaction) (*MenuInstance, error) {
	...
	if tx == nil || !tx.Target.IsValid() || tx.UserID == 0 {
		if tx != nil { _ = tx.Answer(ctx, "Session expired, send /start again", true) }
