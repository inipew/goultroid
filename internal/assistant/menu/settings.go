...
func (c *Controller) AttachSettingsRoutes(r *callback.Router, svc *settings.Service) {
	...
	r.Register("settings", "info", func(ctx context.Context, tx *callback.Transaction) error {
		...
		if !ok || def == nil { _ = tx.Answer(ctx, "Setting is no longer available", true); return fmt.Errorf("unknown setting %s:%s", ns, key) }
	...
	r.Register("settings", "set", func(ctx context.Context, tx *callback.Transaction) error {
		...
		if changed {
			if err := svc.Set(ctx, settings.ScopeUser, tx.UserID, ns, key, value, tx.UserID); err != nil { _ = tx.Answer(ctx, "Invalid setting value", true); return err }
	...
	r.Register("settings", "reset", func(ctx context.Context, tx *callback.Transaction) error {
		...
		if err != nil { return err }
		def, ok := svc.Registry().Get(ns, key)
		if !ok || def == nil { return fmt.Errorf("unknown setting %s:%s", ns, key) }
...
func (c *Controller) beginStringInput(ctx context.Context, tx *callback.Transaction, def *settings.SettingDefinition) error {
	...
	if _, err := tx.Interaction.SendMessage(ctx, tx.Target.Peer(), prompt, nil); err != nil { c.clearPending(tx.UserID); return err }
...
func (c *Controller) HandleTextMessage(ctx context.Context, userID, chatID int64, text string, inter interaction.MessageInteraction, svc *settings.Service) (bool, error) {
	...
	if trimmed == "" { _, _ = inter.SendMessage(ctx, pending.Target.Peer(), "Value cannot be empty.", nil); return true, nil }
	...
	c.clearPending(userID)
	valueText := "updated"
	if !def.Sensitive { valueText = "updated to <code>" + escapeText(trimmed) + "</code>" }
...
func (c *Controller) editScreen(ctx context.Context, tx *callback.Transaction, screen *Screen) error {
	if screen == nil { return fmt.Errorf("settings screen is nil") }
	if c.instances != nil { c.instances.UpdateScreen(tx.Target.ChatID(), tx.Target.MessageID(), ScreenIDSettings) }
	if c.renderer == nil { return tx.Edit(ctx, screen.Text(), nil) }
...
func buildSettingsHome(reg *settings.Registry) *Screen {
	...
	if reg == nil { return screen.AddRow(NewButton("« Back", "a1:assistant:start")) }
	for _, category := range reg.Categories() { screen.AddRow(NewButton(categoryLabel(category), "a1:settings:category:"+category)) }
...
func buildSettingsCategory(ctx context.Context, svc *settings.Service, userID, chatID int64, category string) (*Screen, error) {
	if svc == nil || svc.Registry() == nil { return nil, fmt.Errorf("settings service is unavailable") }
	defs := svc.Registry().ListByCategory(category)
	if len(defs) == 0 { screen := NewScreen(ScreenIDSettings, "⚙️ Settings", "No settings are registered in this category."); screen.AddRow(NewButton("« Settings", "a1:settings:home")); return screen, nil }
...
func buildSettingDetail(ctx context.Context, svc *settings.Service, userID, chatID int64, def settings.SettingDefinition) (*Screen, error) {
	...
	if explicit != nil { status = "User override" }
	...
	if explicit != nil { screen.AddRow(NewButton("↩ Reset", fmt.Sprintf("a1:settings:reset:%s:%s:reset", def.Namespace, def.Key))) }
...
func parseSettingRef(state string) (string, string, error) {
	...
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" { return "", "", fmt.Errorf("invalid setting reference") }
	return parts[0], parts[1], nil
}

func parseSettingState(state string) (string, string, string, error) {
	parts := strings.SplitN(strings.TrimSpace(state), ":", 3)
	if len(parts) < 2 || parts[0] == "" || parts[1] == "" { return "", "", "", fmt.Errorf("invalid setting state") }
	op := "next"
	if len(parts) == 3 && parts[2] != "" { op = parts[2] }
	return parts[0], parts[1], op, nil
}

func nextSettingValue(def *settings.SettingDefinition, current, op string) (string, bool, error) {
	if def == nil { return "", false, fmt.Errorf("nil setting definition") }
	if op != "next" && op != "prev" { return "", false, fmt.Errorf("unsupported setting operation %q", op) }
	direction := int64(1)
	if op == "prev" { direction = -1 }
	switch def.Type {
	case settings.TypeBool:
		v, err := strconv.ParseBool(current); if err != nil { return "", false, err }; return strconv.FormatBool(!v), true, nil
	case settings.TypeEnum:
		if len(def.AllowedValues) == 0 { return "", false, fmt.Errorf("enum %s:%s has no values", def.Namespace, def.Key) }
		for i, v := range def.AllowedValues { if v == current { n := (i + int(direction)) % len(def.AllowedValues); if n < 0 { n += len(def.AllowedValues) }; return def.AllowedValues[n], true, nil } }
		return def.AllowedValues[0], true, nil
	case settings.TypeInt:
		v, err := strconv.ParseInt(current, 10, 64); if err != nil { return "", false, err }; step := def.UI.Step; if step <= 0 { step = 1 }; v += direction * step
		if def.MaxVal != nil && v > *def.MaxVal { if def.MinVal != nil { v = *def.MinVal } else { v = *def.MaxVal } }
		if def.MinVal != nil && v < *def.MinVal { if def.MaxVal != nil { v = *def.MaxVal } else { v = *def.MinVal } }
		return strconv.FormatInt(v, 10), true, nil
	case settings.TypeDuration:
		d, err := time.ParseDuration(current); if err != nil { return "", false, err }; step := time.Duration(def.UI.Step) * time.Second; if step <= 0 { step = 5 * time.Second }; d += time.Duration(direction) * step
		if def.MaxVal != nil && int64(d/time.Second) > *def.MaxVal { d = time.Duration(*def.MaxVal) * time.Second }
		if def.MinVal != nil && int64(d/time.Second) < *def.MinVal { d = time.Duration(*def.MinVal) * time.Second }
...
func displaySettingValue(def settings.SettingDefinition, value string) string { if def.Sensitive && value != "" { return "••••" }; return escapeText(value) }
func boundText(v *int64) string { if v == nil { return "∞" }; return strconv.FormatInt(*v, 10) }
func escapeText(s string) string { return strings.NewReplacer("&", "&amp;", "<", "&lt;", ">", "&gt;", "\"", "&quot;").Replace(s) }
