...
func (p PeerRef) InputPeer() (tg.InputPeerClass, error) {
	if p.ID <= 0 {
		return nil, fmt.Errorf("peer %s: invalid id %d", p.Kind, p.ID)
	}
	switch p.Kind {
	case PeerKindUser:
		if p.AccessHash == 0 {
			return nil, fmt.Errorf("user %d: access hash is required", p.ID)
		}
	...
	case PeerKindChannel:
		if p.AccessHash == 0 {
			return nil, fmt.Errorf("channel %d: access hash is required", p.ID)
		}
		return &tg.InputPeerChannel{ChannelID: p.ID, AccessHash: p.AccessHash}, nil
	default:
		return nil, fmt.Errorf("unsupported peer kind %d", p.Kind)
	}
}
func PeerRefFromInputPeer(p tg.InputPeerClass) (PeerRef, error) {
	...
	case *tg.InputPeerUser:
		if v.AccessHash == 0 {
			return PeerRef{}, fmt.Errorf("user %d: missing access hash", v.UserID)
		}
	...
	case *tg.InputPeerChannel:
		if v.AccessHash == 0 {
			return PeerRef{}, fmt.Errorf("channel %d: missing access hash", v.ChannelID)
		}
