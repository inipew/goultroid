...
type EventMeta struct {
	ID, CorrelationID, CausationID string
}
...
// NewEventBus only constructs the bus. App lifecycle owns Start and Close.
...
// Publish holds the read lock through queue sends. Close takes the write lock,
// so a queue send can never race with close(queue).
...
func (b *EventBus) PublishDurable(ctx context.Context, event Event) error {
	...
	handlers := make([]EventHandler, 0, len(b.subscribers[event.Type()]))
	for _, handler := range b.subscribers[event.Type()] {
