...
func WrapCategory(category ErrorCategory, err error) error { if err == nil { return nil }; return &ErrorWithCategory{Category: category, Err: err} }

func CategoryOf(err error) ErrorCategory {
	if err == nil { return CategoryNone }
	var c *ErrorWithCategory
	if errors.As(err, &c) && c != nil { return c.Category }
	if errors.Is(err, ErrPermissionDenied) { return CategorySecurity }
	if errors.Is(err, ErrInvalidArgs) || errors.Is(err, ErrUnclosedQuote) || errors.Is(err, ErrTrailingEscape) || errors.Is(err, ErrGroupOnly) { return CategoryInvalidInput }
	if errors.Is(err, ErrResourceLimit) { return CategoryResourceLimit }
	if errors.Is(err, ErrRateLimited) { return CategoryRateLimited }
	if errors.Is(err, ErrTimeout) || errors.Is(err, ErrUnavailable) || errors.Is(err, ErrCancelled) || errors.Is(err, ErrConflict) || errors.Is(err, ErrLeaseLost) { return CategoryTransient }
	if errors.Is(err, ErrNotFound) || errors.Is(err, ErrUnsupported) { return CategoryPermanent }
	if strings.Contains(strings.ToUpper(err.Error()), "FLOOD_WAIT") { return CategoryRateLimited }
	if strings.Contains(strings.ToLower(err.Error()), "timeout") || strings.Contains(strings.ToLower(err.Error()), "connection reset") { return CategoryTransient }
	if strings.Contains(strings.ToUpper(err.Error()), "USER_BANNED") { return CategoryPermanent }
