...
type CommandRateLimiter interface { Allow(key string) bool }
...
func (e *CommandExecutor) Execute(ctx *Context, cmd Command) error {
	...
	if strings.HasPrefix(ctx.CorrelationID, "sched-") { source = ExecutionScheduled }
...
func (e *CommandExecutor) ExecuteExecution(exec CommandExecution, cmd Command, svc TelegramServicer) error {
	if exec.Ctx == nil { exec.Ctx = context.Background() }
	if exec.Command == "" { exec.Command = cmd.Name }
	sender := exec.Sender
	if sender == nil && exec.Principal != nil && exec.Principal.ID > 0 { sender = &User{ID: exec.Principal.ID} }
	ctx := &Context{
...
func (e *CommandExecutor) execute(ctx *Context, cmd Command, source ExecutionSource) error {
	...
	chain := NewChain(
		...
		CooldownMiddleware(cmd, e.cooldown), TimeoutMiddleware(cmd, e.defaultTimeout),
	)
	start := time.Now()
	err := chain.Then(cmd.Handler)(ctx)
	if e.metrics != nil { e.metrics.RecordCommand(cmd.Name, time.Since(start), err) }
	if err != nil {
		if errors.Is(err, ErrPermissionDenied) || errors.Is(err, ErrCooldownActive) || errors.Is(err, ErrRateLimited) { return err }
