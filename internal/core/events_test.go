...
func TestEventBus_CallbackQueryEvent(t *testing.T) {
	...
	bus.Subscribe(core.EventTypeCallbackQuery, func(e core.Event) {
		defer wg.Done()
		if cb, ok := e.(*core.CallbackQueryEvent); ok { received = cb }
	})
	now := time.Now()
...
func TestEventBus_PublishDoesNotStarveLaterSubscribersWhenQueueFills(t *testing.T) {
	...
	for later.Load() == 0 {
		select { case <-deadline: t.Fatal("later subscriber was starved"); default: time.Sleep(time.Millisecond) }
	}
}
...
func TestEventBus_Stats(t *testing.T) {
	...
	stats := bus.Stats()
	if stats.Published < 2 { t.Errorf("expected published >= 2, got %d", stats.Published) }
	if stats.Panics != 1 { t.Errorf("expected 1 panic, got %d", stats.Panics) }
	if stats.Delivered < 1 { t.Errorf("expected delivered >= 1, got %d", stats.Delivered) }
}
