...
// Register initializes a plugin and atomically registers all of its commands.
// If initialization succeeds but registration fails, the plugin is shut down
// before returning so partially initialized resources are not leaked.
func (m *Manager) Register(p Plugin) error {
	...
	if err := p.Init(); err != nil {
		if s, ok := p.(Shutdowner); ok { _ = s.Shutdown() }
		if s, ok := p.(ContextShutdowner); ok { _ = s.ShutdownContext(context.Background()) }
		return fmt.Errorf("failed to initialize plugin %s: %w", name, err)
	}

	commands := p.Commands()
	if err := m.router.RegisterBatch(commands); err != nil {
	...
	var meta Metadata
	if dp, ok := p.(DescribedPlugin); ok { meta = dp.Metadata() } else {
...
func (m *Manager) Plugins() []Plugin {
	...
	res := make([]Plugin, len(m.list)); copy(res, m.list); return res
}

func (m *Manager) Find(name string) (Plugin, bool) {
	m.mu.RLock(); defer m.mu.RUnlock()
	p, ok := m.plugins[strings.ToLower(strings.TrimSpace(name))]; return p, ok
}

func (m *Manager) GetMetadata(name string) (Metadata, bool) {
	m.mu.RLock(); defer m.mu.RUnlock()
	meta, ok := m.metadata[strings.ToLower(strings.TrimSpace(name))]; return meta, ok
}

func (m *Manager) AllMetadata() map[string]Metadata {
	m.mu.RLock(); defer m.mu.RUnlock()
	res := make(map[string]Metadata, len(m.metadata)); for k, v := range m.metadata { res[k] = v }; return res
}
...
// context-aware plugin gets the shutdown context. Legacy Shutdowner plugins are
// invoked synchronously because their API cannot be cancelled; this is deliberate
// so shared resources can never be closed while a legacy shutdown is still running.
func (m *Manager) ShutdownWithContext(ctx context.Context) error {
	...
	for i := len(plugins) - 1; i >= 0; i-- {
		...
		select {
		case <-ctx.Done():
			return fmt.Errorf("errors during plugin shutdown: %s", strings.Join(append(errs, fmt.Sprintf("shutdown context cancelled before %s: %v", p.Name(), ctx.Err())), "; "))
		default:
		}
