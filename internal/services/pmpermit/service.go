...
type Service struct {
	...
	maxWarns int
	mu sync.RWMutex
	approvedCache sync.Map // map[int64]approvalCacheEntry
}
...
// IsApproved uses a process-local positive cache. Approved users are common on
// busy accounts, so a DB round trip for every PM is unnecessary. Negative
// states are intentionally not cached to preserve immediate security changes.
func (s *Service) IsApproved(ctx context.Context, userID int64) (bool, error) {
	...
	if rec.ExpiresAt != nil { entry.expiresAt = *rec.ExpiresAt }
...
func (s *Service) Disapprove(ctx context.Context, userID int64) error { s.approvedCache.Delete(userID); return s.db.SetPMStatus(ctx, userID, StatusPending, "approval revoked", nil) }
func (s *Service) Block(ctx context.Context, userID int64, reason string) error { s.approvedCache.Delete(userID); if reason == "" { reason = "blocked by user" }; return s.db.SetPMStatus(ctx, userID, StatusBlocked, reason, nil) }
...
func (s *Service) HandleIncomingPM(ctx context.Context, peer tg.InputPeerClass, senderID int64) (bool, error) {
	...
	if err != nil { s.logger.Warn("pm permit approval check failed", zap.Error(err), zap.Int64("sender_id", senderID)); return false, nil }
	if approved { return false, nil }
	warnCount, err := s.db.IncrementPMWarn(ctx, senderID)
	if err != nil { s.logger.Warn("failed to increment pm warn", zap.Error(err)); return false, nil }
