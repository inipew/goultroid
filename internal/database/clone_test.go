...
func TestCloneRepositoryRoundTrip(t *testing.T) {
	...
	if err := db.SaveCloneState(context.Background(), want); err != nil { t.Fatal(err) }
	got, err := db.GetCloneState(context.Background(), want.OwnerID)
	if err != nil { t.Fatal(err) }
	if got == nil { t.Fatal("expected clone state") }
	...
	if err := db.ClearCloneState(context.Background(), want.OwnerID); err != nil { t.Fatal(err) }
	got, err = db.GetCloneState(context.Background(), want.OwnerID)
	if err != nil { t.Fatal(err) }
	if got != nil { t.Fatalf("expected state to be cleared: %#v", got) }
}
