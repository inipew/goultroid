...
func (d *Dispatcher) dispatch(ctx context.Context, e tg.Entities, msg *tg.Message) error {
	...
	if len(e.Users) > 0 || len(e.Channels) > 0 || len(e.Chats) > 0 {
		...
		if r, ok := resolver.(*Resolver); ok && r.storage != nil {
			...
			for _, u := range e.Users { users = append(users, u) }
			channels := make([]*tg.Channel, 0, len(e.Channels))
			for _, ch := range e.Channels { channels = append(channels, ch) }
			chats := make([]*tg.Chat, 0, len(e.Chats))
			for _, c := range e.Chats { chats = append(chats, c) }
		...
		if p.UserID == d.getSelfID() {
			...
			if accessHash == 0 && d.getResolver() != nil {
				if resolved, _, err := d.getResolver().ResolveUser(ctx, strconv.FormatInt(p.UserID, 10)); err == nil {
					if ipu, ok := resolved.(*tg.InputPeerUser); ok && ipu.AccessHash != 0 { accessHash = ipu.AccessHash }
				}
			}
			if accessHash != 0 || d.getResolver() == nil { peerInput = &tg.InputPeerUser{UserID: p.UserID, AccessHash: accessHash} }
		}
	case *tg.PeerChat:
		chat.ID = p.ChatID
		chat.Type = "group"
		peerInput = &tg.InputPeerChat{ChatID: p.ChatID}
		if c, ok := e.Chats[p.ChatID]; ok { chat.Title = c.Title }
	case *tg.PeerChannel:
		chat.ID = p.ChannelID
		...
		if ch, ok := e.Channels[p.ChannelID]; ok {
			...
			if ch.Megagroup { chat.Type = "supergroup" } else { chat.Type = "channel" }
			accessHash = ch.AccessHash
		}
		if accessHash == 0 && d.getResolver() != nil {
			if resolved, err := d.getResolver().ResolveChat(ctx, fmt.Sprintf("-100%d", p.ChannelID)); err == nil {
				if ipc, ok := resolved.(*tg.InputPeerChannel); ok && ipc.AccessHash != 0 { accessHash = ipc.AccessHash }
			}
		}
	...
	if peerInput == nil && msg.PeerID != nil {
		d.logger.Warn("dispatch: peer unresolvable without access hash, command execution skipped", zap.String("peerType", fmt.Sprintf("%T", msg.PeerID)), zap.Int64("chatID", chat.ID), zap.String("command", cmdName))
		return nil
	}
	...
	if root == nil { if d.logger != nil { d.logger.Warn("dispatcher: root context is nil, falling back to background - lifecycle not correctly wired") }; root = context.Background() }
...
func extractCoreMessage(msg *tg.Message) *core.Message {
	coreMsg := &core.Message{ID: msg.ID, Text: msg.Message, Date: time.Unix(int64(msg.Date), 0), IsOutgoing: msg.Out, GroupedID: msg.GroupedID, Entities: msg.Entities}
	if msg.Media != nil { coreMsg.Media = core.ExtractMediaFromTG(msg.Media); if coreMsg.Media != nil { coreMsg.MediaType = coreMsg.Media.Type } }
	if msg.ReplyTo != nil { if header, ok := msg.ReplyTo.(*tg.MessageReplyHeader); ok { coreMsg.ReplyToID = header.ReplyToMsgID; if header.ForumTopic || header.ReplyToTopID != 0 { if header.ReplyToTopID != 0 { coreMsg.TopicID = header.ReplyToTopID } else { coreMsg.TopicID = header.ReplyToMsgID } } } }
	return coreMsg
}
