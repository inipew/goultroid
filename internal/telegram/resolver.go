...
// ResolveUser resolves a user reference into an InputPeer and User ID.
// For arbitrary numeric IDs it fails closed when no usable access hash is available.
func (r *Resolver) ResolveUser(ctx context.Context, ref string) (tg.InputPeerClass, int64, error) {
	...
	if r.api != nil {
		resolved, err := r.api.ContactsResolveUsername(ctx, &tg.ContactsResolveUsernameRequest{Username: cleaned})
	...
	if r.api != nil {
		...
		if err == nil && resolved != nil {
			for _, c := range resolved.Chats {
				...
				case *tg.Channel:
					if ch.AccessHash == 0 {
						return nil, fmt.Errorf("%w: channel %d returned without access hash", core.ErrAccessHashMissing, ch.ID)
					}
					if r.storage != nil {
