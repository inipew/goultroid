...
func styledSegments(text string, entities []tg.MessageEntityClass) []styledSegment {
	...
	for _, entity := range entities {
		...
		style := styleNormal
		switch entity.(type) {
		case *tg.MessageEntityPre, *tg.MessageEntityPreCode, *tg.MessageEntityCode: style = styleCode
		...
		for i := start; i < end && i < len(styles); i++ {
			if style == styleCode || styles[i] == styleNormal || style == styleBold || style == styleItalic { styles[i] = style }
		}
	}
	var out []styledSegment; start := 0
	for i := 1; i <= len(runes); i++ {
		if i == len(runes) || styles[i] != styles[start] { out = append(out, styledSegment{text: string(runes[start:i]), style: styles[start]}); start = i }
	}
	return out
}
...
func entityRuneRange(text string, entity tg.MessageEntityClass) (int, int, bool) {
	...
	case *tg.MessageEntityPreCode: offset, length = e.Offset, e.Length
...
func drawAvatar(dst draw.Image,avatar image.Image,center image.Point,radius int){
	for y:=-radius;y<=radius;y++{for x:=-radius;x<=radius;x++{if x*x+y*y>radius*radius{continue};sx:=(x+radius)*avatar.Bounds().Dx()/(radius*2);sy:=(y+radius)*avatar.Bounds().Dy()/(radius*2);dst.Set(center.X+x,center.Y+y,avatar.At(avatar.Bounds().Min.X+sx,avatar.Bounds().Min.Y+sy))}}
}
