...
func renderV3(path, name, text string, msg *core.Message, mediaPath, avatarPath string) error {
	...
	bodyFace := loadFont(29)
	boldFace := loadFont(29)
	italicFace := loadFont(29)
	codeFace := loadFont(26)
	metaFace := loadFont(19)
	textWidth := width - pad*2

	if strings.TrimSpace(text) == "" {
		text = mediaPlaceholder(msg.MediaType)
	}
	segments := styledSegments(text, msg.Entities)
	lines := wrapStyledSegments(segments, bodyFace, textWidth)
	if len(lines) == 0 {
		lines = []styledLine{{segments: []styledSegment{{text: mediaPlaceholder(msg.MediaType)}}}}
	}

	media, mediaKind := loadQuoteMedia(mediaPath, msg.Media)
	mediaHeight := 0
	if media != nil {
		mediaHeight = media.Bounds().Dy() + 34
	...
	nameX := 176
	d := &font.Drawer{Dst: img, Face: nameFace, Src: image.NewUniform(theme.primary), Dot: fixedPoint(nameX, 101)}
	...
	for _, line := range lines {
		drawStyledLine(img, line, pad, y, bodyFace, boldFace, italicFace, codeFace, theme)
		y += lineHeight
	}

	if media != nil {
		y += 18
		x := (width - media.Bounds().Dx()) / 2
		drawRoundedImage(img, media, image.Pt(x, y), 20)
	} else if msg.Media != nil {
		y += 12
		drawMediaCard(img, image.Rect(pad, y, width-pad, y+90), msg.Media, mediaKind, theme, metaFace)
	}
...
func styledSegments(text string, entities []tg.MessageEntityClass) []styledSegment {
	...
	for _, entity := range entities {
		...
		style := styleNormal
		switch entity.(type) {
		case *tg.MessageEntityPre, *tg.MessageEntityPreCode, *tg.MessageEntityCode:
			style = styleCode
		...
		for i := start; i < end && i < len(styles); i++ {
			// Code wins over all other presentation styles, then bold/italic/link.
...
func entityRuneRange(text string, entity tg.MessageEntityClass) (int, int, bool) {
	...
	runesBeforeStart := len([]rune(text[:start]))
	runesBeforeEnd := len([]rune(text[:end]))
	return runesBeforeStart, runesBeforeEnd, true
}
...
func drawStyledLine(dst draw.Image, line styledLine, x, y int, normal, bold, italic, code font.Face, theme quoteTheme) {
	d := &font.Drawer{Dst: dst, Dot: fixedPoint(x, y)}
	for _, seg := range line.segments {
		...
		case styleBold:
			d.Face = bold
			d.Src = image.NewUniform(theme.primary)
		case styleItalic:
			d.Face = italic
			d.Src = image.NewUniform(theme.primary)
		case styleCode:
...
func loadAvatar(path string, size int, fallback color.Color) image.Image {
	...
	draw.Draw(img, img.Bounds(), &image.Uniform{C: fallback}, image.Point{}, draw.Src)
	return img
}
...
func drawAvatar(dst draw.Image, avatar image.Image, center image.Point, radius int) {
	for y := -radius; y <= radius; y++ {
		for x := -radius; x <= radius; x++ {
			...
			sx := (x + radius) * avatar.Bounds().Dx() / (radius * 2)
			sy := (y + radius) * avatar.Bounds().Dy() / (radius * 2)
			dst.Set(center.X+x, center.Y+y, avatar.At(avatar.Bounds().Min.X+sx, avatar.Bounds().Min.Y+sy))
		}
	}
}
...
func drawMediaCard(dst draw.Image, r image.Rectangle, media *core.MediaInfo, kind string, theme quoteTheme, face font.Face) {
	drawRoundedRect(dst, r, 20, color.RGBA{38, 40, 48, 255})
	icon := mediaPlaceholder(kind)
	d := &font.Drawer{Dst: dst, Face: loadFont(26), Src: image.NewUniform(theme.accent), Dot: fixedPoint(r.Min.X+28, r.Min.Y+42)}
	d.DrawString(icon)
	meta := ""
	if media != nil {
		if media.FileName != "" {
	...
	if meta != "" {
		d.Face = face
		d.Src = image.NewUniform(theme.muted)
		d.Dot = fixedPoint(r.Min.X+28, r.Min.Y+72)
		d.DrawString(truncateToWidth(meta, face, r.Dx()-56))
	}
}
...
func drawRoundedImage(dst draw.Image, src image.Image, at image.Point, radius int) {
	...
	for y := r.Min.Y; y < r.Max.Y; y++ {
		for x := r.Min.X; x < r.Max.X; x++ {
			localX := x - r.Min.X
			localY := y - r.Min.Y
			if !insideRounded(localX, localY, r.Dx(), r.Dy(), radius) {
				continue
			}
			dst.Set(at.X+localX, at.Y+localY, src.At(x, y))
		}
	}
}
...
func fixedPoint(x, y int) fixed.Int26_6 { return fixed.P(x, y) }
