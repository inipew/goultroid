...
func drawAvatar(dst draw.Image, avatar image.Image, center image.Point, radius int) {
	for y := -radius; y <= radius; y++ {
		for x := -radius; x <= radius; x++ {
			...
			sx := (x + radius) * avatar.Bounds().Dx() / (radius*2 + 1)
			sy := (y + radius) * avatar.Bounds().Dy() / (radius*2 + 1)
			if sx >= avatar.Bounds().Dx() {
				sx = avatar.Bounds().Dx() - 1
			}
			if sy >= avatar.Bounds().Dy() {
				sy = avatar.Bounds().Dy() - 1
			}
