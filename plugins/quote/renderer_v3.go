...
func renderV3(path, name, text string, msg *core.Message, mediaPath, avatarPath string) error {
	...
	if media != nil {
		y += 18
		drawRoundedImage(img, media, image.Pt((width-media.Bounds().Dx())/2, y), 20)
	} else if msg.Media != nil {
		y += 12
...
func drawMediaCard(dst draw.Image, r image.Rectangle, media *core.MediaInfo, kind string, theme quoteTheme, face font.Face) {
	...
	d := &font.Drawer{Dst: dst, Face: loadFont(26), Src: image.NewUniform(theme.accent), Dot: fixed.P(r.Min.X+28, r.Min.Y+42)}
	...
	if meta != "" {
		...
		d.Dot = fixed.P(r.Min.X+28, r.Min.Y+72)
