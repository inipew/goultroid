...
func (p *Plugin) handleWhois(ctx *core.Context) error {
	...
	if err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to fetch user info: %v", err)) }
	var u *tg.User
	for _, userClass := range fullUser.Users { if usr, ok := userClass.(*tg.User); ok { u = usr; break } }
	if u == nil { return ctx.EditOrReply("❌ Could not parse user details.") }
	...
	if u.Verified { sb.WriteString("• <b>Verified</b>: Yes ✅\n") }
	if u.Scam { sb.WriteString("• <b>Scam</b>: ⚠️ Yes\n") }
	if u.Fake { sb.WriteString("• <b>Fake</b>: ⚠️ Yes\n") }
	if u.Restricted { sb.WriteString("• <b>Restricted</b>: Yes ⛔\n") }
	if fullUser.FullUser.CommonChatsCount > 0 { sb.WriteString(fmt.Sprintf("• <b>Common Chats</b>: %d\n", fullUser.FullUser.CommonChatsCount)) }
	if fullUser.FullUser.About != "" { sb.WriteString(fmt.Sprintf("• <b>Bio</b>: <code>%s</code>\n", core.EscapeHTML(fullUser.FullUser.About))) }
	return ctx.EditOrReply(sb.String())
}

func (p *Plugin) handleChatInfo(ctx *core.Context) error {
	fullChat, err := ctx.GetFullChat()
	if err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to fetch chat info: %v", err)) }
	var sb strings.Builder
	sb.WriteString("👥 <b>Chat Information</b>\n\n")
	title := "Unknown"
	if ctx.Chat != nil && ctx.Chat.Title != "" { title = ctx.Chat.Title }
	var chatID int64
	...
	case *tg.ChannelFull:
		...
		if ch.AdminsCount > 0 { sb.WriteString(fmt.Sprintf("• <b>Admins</b>: %d\n", ch.AdminsCount)) }
		if ch.BannedCount > 0 { sb.WriteString(fmt.Sprintf("• <b>Banned</b>: %d\n", ch.BannedCount)) }
		if ch.KickedCount > 0 { sb.WriteString(fmt.Sprintf("• <b>Kicked</b>: %d\n", ch.KickedCount)) }
		if ch.SlowmodeSeconds > 0 { sb.WriteString(fmt.Sprintf("• <b>Slowmode</b>: %ds\n", ch.SlowmodeSeconds)) }
		if ch.About != "" { sb.WriteString(fmt.Sprintf("• <b>Description</b>: <i>%s</i>\n", core.EscapeHTML(ch.About))) }
	case *tg.ChatFull:
		if ch.About != "" { sb.WriteString(fmt.Sprintf("• <b>Description</b>: <i>%s</i>\n", core.EscapeHTML(ch.About))) }
		if participants, ok := ch.Participants.(*tg.ChatParticipants); ok { sb.WriteString(fmt.Sprintf("• <b>Members</b>: %d\n", len(participants.Participants))) }
	}
	return ctx.EditOrReply(sb.String())
}
...
func resolveInputUser(ctx *core.Context) (tg.InputUserClass, error) {
	if len(ctx.Args) > 0 {
		...
		if resErr != nil { return nil, fmt.Errorf("failed to resolve user %q: %w", arg, resErr) }
		for _, u := range resolved.Users {
			if usr, ok := u.(*tg.User); ok { return &tg.InputUser{UserID: usr.ID, AccessHash: usr.AccessHash}, nil }
		}
...
func (p *Plugin) handleID(ctx *core.Context) error {
	...
	if ctx.Message != nil && ctx.Message.ReplyToID != 0 {
		...
		if reply, err := ctx.GetReply(); err == nil && reply != nil && reply.SenderID != 0 { sb.WriteString(fmt.Sprintf("• <b>Reply Sender ID:</b> <code>%d</code>\n", reply.SenderID)) }
	}
	return ctx.EditOrReply(sb.String())
}
