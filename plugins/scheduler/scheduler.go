...
func (p *Plugin) handleRemind(ctx *core.Context) error {
	if len(ctx.Args) == 0 {
		_ = ctx.Status("Usage: <code>.remind &lt;duration&gt; &lt;text&gt;</code> or reply to a message with <code>.remind &lt;duration&gt;</code>\nExample: <code>.remind 15m Take a break</code>")
	...
	if len(ctx.Args) >= 2 {
		...
		if err != nil || reply == nil || reply.Text == "" {
			_ = ctx.Status("Please specify reminder text or reply to a text message.")
	...
	return ctx.Success(fmt.Sprintf("Reminder set for <code>%s</code>. Job ID: <code>#%d</code>.", durStr, job.ID))
}

func (p *Plugin) handleSchedule(ctx *core.Context) error {
	if len(ctx.Args) < 2 {
		_ = ctx.Status("Usage: <code>.schedule [in|every] &lt;duration&gt; &lt;text/command&gt;</code>\nExamples:\n• <code>.schedule in 30m .whois @user</code>\n• <code>.schedule every 2h .alive</code>")
		return core.NewUsageError("missing arguments")
	}
	...
	if len(ctx.Args) <= payloadIdx {
		_ = ctx.Status("Please provide a text or command payload to schedule.")
	...
	if payload == "" {
		_ = ctx.Status("Payload cannot be empty.")
...
func (p *Plugin) handleList(ctx *core.Context) error {
	...
	if len(jobs) == 0 {
		return ctx.Status("No active scheduled jobs in this chat.")
	}
...
func (p *Plugin) handleCancel(ctx *core.Context) error {
	if len(ctx.Args) == 0 {
		_ = ctx.Status("Usage: <code>.cancelschedule &lt;id&gt;</code>")
...
func (p *Plugin) handleSchedHistory(ctx *core.Context) error {
	if len(ctx.Args) == 0 {
		_ = ctx.Status("Usage: <code>.schedhistory &lt;id&gt; [limit]</code>")
		return core.NewUsageError("missing job id")
	}
	idStr := strings.TrimPrefix(ctx.Args[0], "#")
	...
	if len(entries) == 0 {
		return ctx.Status(fmt.Sprintf("No execution history found for job <code>#%d</code>.", jobID))
	}
	var sb strings.Builder
