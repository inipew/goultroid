...
func (p *Plugin) Init() error {
	if p.repo == nil { return errors.New("clone repository is not initialized") }
	if p.ownerID <= 0 { return errors.New("clone owner ID is not configured") }
	return os.MkdirAll(cloneDataDir, 0700)
}
...
func (p *Plugin) handleClone(ctx *core.Context) error {
	...
	if err := p.repo.SaveCloneState(ctx.Ctx, snapshot); err != nil {
		if snapshotPath != "" { _ = os.Remove(snapshotPath) }
	...
	if clonedPhoto {
		...
		uploadErr := ctx.Svc.UploadProfilePhoto(ctx.Ctx, path)
		_ = os.Remove(path)
...
func (p *Plugin) handleRevert(ctx *core.Context) error {
	...
	if state.ClonedPhoto {
		if _, err := ctx.Svc.DeleteProfilePhotos(ctx.Ctx, 1); err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to remove cloned profile photo: %v", err)) }
	}
...
func (p *Plugin) resolveTarget(ctx *core.Context) (tg.InputPeerClass, int64, tg.InputUserClass, *tg.User, *tg.UsersUserFull, error) {
	...
	if err != nil { return nil, 0, nil, nil, nil, err }
	inputPeer, ok := peer.(*tg.InputPeerUser)
	if !ok || inputPeer == nil || inputPeer.AccessHash == 0 { return nil, 0, nil, nil, nil, errors.New("resolved target does not contain a usable user access hash") }
	inputUser := &tg.InputUser{UserID: inputPeer.UserID, AccessHash: inputPeer.AccessHash}
	full, err := ctx.Peer().GetFullUser(inputUser)
	if err != nil { return nil, 0, nil, nil, nil, fmt.Errorf("failed to fetch target profile: %w", err) }
	var user *tg.User
	for _, item := range full.Users { if u, ok := item.(*tg.User); ok && u.ID == id { user = u; break } }
...
func (p *Plugin) downloadProfilePhoto(ctx *core.Context, peer tg.InputPeerClass, photoID int64, prefix string) (string, error) {
	if ctx.Svc == nil { return "", errors.New("telegram service is not initialized") }
	if photoID == 0 { return "", errors.New("profile photo has no ID") }
	...
	if err := ctx.Svc.DownloadFile(ctx.Ctx, location, path); err != nil { _ = os.Remove(path); return "", err }
	return path, nil
}

func (p *Plugin) cloneFailure(ctx *core.Context, snapshot database.CloneState, cause error) error {
	if ctx.Svc != nil {
		if snapshot.ClonedPhoto { _, _ = ctx.Svc.DeleteProfilePhotos(ctx.Ctx, 1) }
		_ = ctx.UpdateProfile(&snapshot.OriginalFirst, &snapshot.OriginalLast, &snapshot.OriginalBio)
		if snapshot.OriginalPhoto != "" { _ = ctx.Svc.UploadProfilePhoto(ctx.Ctx, snapshot.OriginalPhoto) }
	}
