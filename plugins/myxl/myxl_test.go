...
func TestMyXLPlugin_Commands(t *testing.T) {
	...
	// 5. .myxl accounts (should show 1 active account)
	ctxAcc2 := *baseCtx
	ctxAcc2.Args = []string{"accounts"}
	_ = cmdMap["myxl"].Handler(&ctxAcc2)
	if !strings.Contains(svc.sent, "6281912345678") || !strings.Contains(svc.sent, "AKTIF") {
		t.Errorf("expected active account listed, got %s", svc.sent)
	}
