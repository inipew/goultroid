...
func (p *Plugin) Commands() []core.Command {
	return []core.Command{
		{Name: "ban", Description: "Ban a user from the group", Usage: ".ban <user_id / reply> [reason]", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleBan},
		{Name: "unban", Description: "Unban a user in the group", Usage: ".unban <user_id / reply>", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleUnban},
		{Name: "kick", Description: "Kick a user from the group", Usage: ".kick <user_id / reply>", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleKick},
		{Name: "mute", Description: "Mute a user in the group (optional duration, e.g. 10m, 2h, 1d)", Usage: ".mute <user_id / reply> [duration]", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleMute},
		{Name: "unmute", Description: "Unmute a user in the group", Usage: ".unmute <user_id / reply>", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleUnmute},
		{Name: "purge", Description: "Safely delete messages between replied message and this command (topic-aware)", Usage: "reply to a message with .purge", Category: "Admin", Permission: core.PermissionSudo, ReplyOnly: true, Handler: p.handlePurge},
		{Name: "promote", Description: "Promote a user to admin with custom title", Usage: ".promote <user_id / reply> [title]", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handlePromote},
		{Name: "demote", Description: "Demote an admin back to regular user", Usage: ".demote <user_id / reply>", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleDemote},
		{Name: "warn", Description: "Add a warning to a user (auto-punishes when threshold reached)", Usage: ".warn <user_id / reply> [reason]", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleWarn},
		{Name: "warns", Description: "View active warnings for a user", Usage: ".warns <user_id / reply>", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleWarns},
		{Name: "resetwarns", Description: "Reset all warnings for a user", Usage: ".resetwarns <user_id / reply>", Category: "Admin", Permission: core.PermissionSudo, GroupOnly: true, Handler: p.handleResetWarns},
	}
}
...
func (p *Plugin) handleKick(ctx *core.Context) error {
	if isPrivateOrUnsupported(ctx) {
		_ = ctx.EditOrReply("⚠️ Fitur kick hanya dapat digunakan di grup or supergroup.")
		return core.ErrUnsupported
	}
	targetPeer, targetID, err := ctx.ResolveTargetUser()
	if err != nil {
...
func (p *Plugin) handleMute(ctx *core.Context) error {
	if isPrivateOrUnsupported(ctx) {
		_ = ctx.EditOrReply("⚠️ Fitur mute hanya dapat digunakan di grup or supergroup.")
		return core.ErrUnsupported
	}
	targetPeer, targetID, err := ctx.ResolveTargetUser()
	if err != nil {
...
func (p *Plugin) handleUnmute(ctx *core.Context) error {
	if isPrivateOrUnsupported(ctx) {
		_ = ctx.EditOrReply("⚠️ Fitur unmute hanya dapat digunakan di grup or supergroup.")
		return core.ErrUnsupported
	}
	targetPeer, targetID, err := ctx.ResolveTargetUser()
	if err != nil {
...
func (p *Plugin) handlePurge(ctx *core.Context) error {
	...
	// this is a bot update or an outgoing userbot command. Do not use
	// EditOrReply here: outgoing commands are normally edited in place, which
	// would leave `.purge` behind after the target range has been deleted.
...
func (p *Plugin) handlePromote(ctx *core.Context) error {
	if isPrivateOrUnsupported(ctx) {
		_ = ctx.EditOrReply("⚠️ Fitur promote hanya dapat digunakan di grup or supergroup.")
		return core.ErrUnsupported
	}
	targetPeer, targetID, err := ctx.ResolveTargetUser()
	if err != nil {
...
func (p *Plugin) handleDemote(ctx *core.Context) error {
	...
	return ctx.EditOrReply(fmt.Sprintf("👤 Demoted user <code>%d</code>.", targetID))
}

func (p *Plugin) handleWarn(ctx *core.Context) error {
	if isPrivateOrUnsupported(ctx) {
		_ = ctx.EditOrReply("⚠️ Fitur warn hanya dapat digunakan di grup or supergroup.")
		return core.ErrUnsupported
	}
	if p.moderator == nil {
	...
	count, err := p.moderator.AddWarning(ctx.Ctx, ctx.ChatID(), targetID, reason)
	if err != nil {
		_ = ctx.EditOrReply(formatAdminError("add warning", err))
		return err
	}
	return ctx.EditOrReply(fmt.Sprintf("⚠️ User <code>%d</code> warned. Total warnings: <b>%d</b>.", targetID, count))
}

func (p *Plugin) handleWarns(ctx *core.Context) error {
	if isPrivateOrUnsupported(ctx) {
		_ = ctx.EditOrReply("⚠️ Fitur warns hanya dapat digunakan di grup or supergroup.")
		return core.ErrUnsupported
	}
	if p.moderator == nil {
		_ = ctx.EditOrReply("⚠️ Moderation service is not configured.")
	...
	warnings, err := p.moderator.GetWarnings(ctx.Ctx, ctx.ChatID(), targetID)
	if err != nil {
		_ = ctx.EditOrReply(formatAdminError("get warnings", err))
		return err
	}
	if len(warnings) == 0 {
		return ctx.EditOrReply(fmt.Sprintf("ℹ️ User <code>%d</code> has no active warnings.", targetID))
	}
	return ctx.EditOrReply(fmt.Sprintf("⚠️ User <code>%d</code> has <b>%d</b> active warnings.", targetID, len(warnings)))
}

func (p *Plugin) handleResetWarns(ctx *core.Context) error {
	if isPrivateOrUnsupported(ctx) {
		_ = ctx.EditOrReply("⚠️ Fitur resetwarns hanya dapat digunakan di grup or supergroup.")
		return core.ErrUnsupported
	}
	if p.moderator == nil {
		_ = ctx.EditOrReply("⚠️ Moderation service is not configured.")
	...
	if err := p.moderator.ResetWarnings(ctx.Ctx, ctx.ChatID(), targetID); err != nil {
		_ = ctx.EditOrReply(formatAdminError("reset warnings", err))
		return err
	}
	return ctx.EditOrReply(fmt.Sprintf("🧹 Warnings cleared for user <code>%d</code>.", targetID))
}
