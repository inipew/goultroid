...
func (p *Plugin) Description() string { return "Anti-spam shield and private message access control system" }
...
func (p *Plugin) HandleIncomingMessage(ctx context.Context, e tg.Entities, msg *tg.Message, isCommand bool, cmdName string) error {
	...
	if msg.Out {
		...
		if p.svc.IsPMPermitMessage(msg.Message) || p.svc.IsBotSent(msg.ID) { return nil }
		if peerUser, ok := msg.PeerID.(*tg.PeerUser); ok && peerUser != nil {
			targetID := peerUser.UserID
			if p.svc.IsWarnID(targetID, msg.ID) { return nil }
			if targetID != 0 && targetID != p.svc.OwnerID() && !p.svc.IsSudoID(targetID) {
				...
				if err := p.svc.AutoApproveOutgoing(ctx, peer, targetID); err != nil { return err }
			}
		}
		return nil
	}
	...
	if msg.FromID != nil {
		if u, ok := msg.FromID.(*tg.PeerUser); ok { senderID = u.UserID }
	}
	...
	if handled {
		if isCommand { core.MarkMessageHandled(senderID, msg.ID) }
		if decision := core.GetMessageDecision(ctx); decision != nil {
			decision.SetHandled(true); decision.SetSuppressAutomation(true); decision.SetSuppressAFK(true); decision.SetSuppressFilters(true); decision.SetSuppressCommands(true)
		}
...
func (p *Plugin) resolveTargetUser(ctx *core.Context) (tg.InputPeerClass, int64) {
	if peer, id, err := ctx.ResolveTargetUser(); err == nil && id != 0 { return peer, id }
	if u, ok := ctx.PeerID.(*tg.InputPeerUser); ok && u != nil && u.UserID != 0 && u.AccessHash != 0 { return u, u.UserID }
	return nil, 0
}
...
func (p *Plugin) handleApprove(ctx *core.Context) error {
	...
	if target == 0 { return ctx.EditOrReply("⚠️ Could not determine user. Reply to a message, run inside a PM, or provide user ID / @username.") }
	if peer == nil { return ctx.EditOrReply("⚠️ Could not resolve a usable Telegram peer for this user.") }
	reason := "Approved by owner"
	if len(ctx.Args) > 1 { reason = strings.Join(ctx.Args[1:], " ") }
	if err := p.svc.ApproveWithPeer(ctx.Ctx, peer, target, reason, 0); err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to approve user: %v", err)) }
...
func (p *Plugin) handleBlock(ctx *core.Context) error {
	...
	if len(ctx.Args) > 1 { reason = strings.Join(ctx.Args[1:], " ") }
	if err := p.svc.BlockWithPeer(ctx.Ctx, peer, target, reason); err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to block user: %v", err)) }
...
func (p *Plugin) handleUnblock(ctx *core.Context) error {
	...
	peer, target := p.resolveTargetUser(ctx)
	if target == 0 { return ctx.EditOrReply("⚠️ Could not determine user. Reply to a message, run inside a PM, or provide user ID / @username.") }
	if err := p.svc.Unblock(ctx.Ctx, peer, target); err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to unblock user: %v", err)) }
...
func (p *Plugin) renderList(ctx *core.Context, statusFilter string) error {
	...
	var err error
	switch statusFilter {
	case "approved": records, err = p.svc.ListApproved(ctx.Ctx, 50, 0)
	case "blocked": records, err = p.svc.ListBlocked(ctx.Ctx, 50, 0)
	case "pending": records, err = p.svc.ListPending(ctx.Ctx, 50, 0)
	default: return ctx.EditOrReply("⚠️ Invalid status filter. Use <code>approved</code>, <code>blocked</code>, or <code>pending</code>.")
	}
	if err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to list records: %v", err)) }
	if len(records) == 0 { return ctx.EditOrReply(fmt.Sprintf("📋 No <b>%s</b> PM records found.", statusFilter)) }
	...
	for i, r := range records {
		...
		if r.Reason != "" { sb.WriteString(fmt.Sprintf(" — <i>%s</i>", r.Reason)) }
		sb.WriteString("\n")
		if i >= 40 { sb.WriteString(fmt.Sprintf("<i>... and %d more</i>\n", len(records)-40)); break }
	}
...
func (p *Plugin) handleToggle(ctx *core.Context) error {
	if p.svc == nil { return ctx.EditOrReply("⚠️ PM Permit service is not configured.") }
	sub := ""
	if len(ctx.Args) > 0 { sub = strings.ToLower(ctx.Args[0]) }
	switch sub {
	case "on", "enable", "true": p.svc.SetEnabled(true); return ctx.EditOrReply("🛡️ <b>PM Permit</b> is now <b>ENABLED</b>.")
	case "off", "disable", "false": p.svc.SetEnabled(false); return ctx.EditOrReply("⚠️ <b>PM Permit</b> is now <b>DISABLED</b>.")
	case "unblock":
		if len(ctx.Args) < 2 { return ctx.EditOrReply("⚠️ Usage: <code>.pmpermit unblock <user_id / @username / reply></code>") }
		subCtx := *ctx; subCtx.Args = ctx.Args[1:]
		peer, target := p.resolveTargetUser(&subCtx)
		if target == 0 { return ctx.EditOrReply("⚠️ Could not determine user to unblock.") }
		if err := p.svc.Unblock(ctx.Ctx, peer, target); err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to unblock user: %v", err)) }
		return ctx.EditOrReplyWithDelay(fmt.Sprintf("✅ <b>Unblocked</b> user <code>%d</code>.", target), 4*time.Second)
	case "list":
		statusFilter := "approved"; if len(ctx.Args) > 1 { statusFilter = strings.ToLower(ctx.Args[1]) }
		return p.renderList(ctx, statusFilter)
	case "test":
		pending, approved, blocked, err := p.svc.GetStats(ctx.Ctx)
		if err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ PM Permit test failed: %v", err)) }
	...
	case "status", "":
		statusStr := "❌ DISABLED"; if p.svc.IsEnabled() { statusStr = "🛡️ ENABLED" }
		pending, approved, blocked, err := p.svc.GetStats(ctx.Ctx)
		if err != nil { return ctx.EditOrReply(fmt.Sprintf("❌ Failed to read PM Permit status: %v", err)) }
	...
	default: return ctx.EditOrReply("⚠️ Unknown option. Usage: <code>.pmpermit [on|off|status|test|list|unblock]</code>")
	}
}
